...
- [x] 补全用户信息，健全原有功能，优化代码逻辑等（20250725）
- [x] 文件组支持选择公开和私有，支持超级管理员和用户分享文件组（20250728）
- [ ] 支持根据URL信息添加RAG文本且处理其中图片，支持word文件中图片处理
- [ ] 添加统一的枚举管理表，配合修改当前页面中的枚举信息。
- [ ] 可通过对话无感添加RAG文本，形成一个特有的用户文件知识库空间
- [ ] 当单向量库数据多的时候，需要根据不同的文件组，切换不同的向量库
