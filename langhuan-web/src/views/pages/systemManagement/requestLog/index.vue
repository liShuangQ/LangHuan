<template>
    <div class="request-log-container">
        <!-- 查询表单 -->
        <div class="search-form-wrapper mb-4 bg-white rounded-lg shadow-sm">
            <ElementFormC ref="searchFormRef" :formConfig="formConfig" :formItemConfig="formItemConfig"
                @handle="formHandle">
        ...
        <!-- 统计信息 -->
        <!-- <div class="statistics-wrapper mb-4 p-4 bg-white rounded-lg shadow-sm">
                ...
                <template #expand="props">
                    <div class="p-4 bg-gray-50">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <h4 class="font-semibold mb-2 text-gray-700">接口信息</h4>
                                <p><span class="font-medium">接口说明：</span>{{ props.row.apiDescription || '无' }}</p>
                                <p><span class="font-medium">请求IP：</span>{{ props.row.requestIp }}</p>
                            </div>
                            <div>
                                <h4 class="font-semibold mb-2 text-gray-700">执行信息</h4>
                                <p><span class="font-medium">用户ID：</span>{{ props.row.userId }}</p>
                                <p><span class="font-medium">错误信息：</span>{{ props.row.errorMessage || '无' }}</p>
                            </div>
                        </div>
                        <div class="mt-4">
                            <h4 class="font-semibold mb-2 text-gray-700">请求参数</h4>
                            <pre
                                class="bg-white p-2 rounded border text-sm overflow-auto max-h-32">{{ formatJsonData(props.row.requestParams) }}</pre>
                        </div>
                        <div class="mt-4">
                            <h4 class="font-semibold mb-2 text-gray-700">响应数据</h4>
                            <pre
                                class="bg-white p-2 rounded border text-sm overflow-auto max-h-32">{{ formatJsonData(props.row.responseData) }}</pre>
                        </div>
                    </div>
                </template>
            </ElementTableC>
        ...
        <!-- 详情对话框 -->
        <el-dialog v-model="detailDialogVisible" title="接口调用详情" width="800px" :close-on-click-modal="false">
            <div v-if="currentDetailRow">
                <el-descriptions :column="2" border>
                    <el-descriptions-item label="接口名称">{{ currentDetailRow.apiName }}</el-descriptions-item>
                    <el-descriptions-item label="接口URL">{{ currentDetailRow.apiUrl }}</el-descriptions-item>
                    <el-descriptions-item label="HTTP方法">
                        <el-tag :type="getHttpMethodType(currentDetailRow.httpMethod) as any" size="small">
                            {{ currentDetailRow.httpMethod }}
                        </el-tag>
                    </el-descriptions-item>
                    <el-descriptions-item label="执行状态">
                        <el-tag :type="currentDetailRow.isSuccess ? 'success' : 'danger'" size="small">
                            {{ currentDetailRow.isSuccess ? '成功' : '失败' }}
                        </el-tag>
                    </el-descriptions-item>
                    <el-descriptions-item label="用户名">{{ currentDetailRow.username }}</el-descriptions-item>
                ...
                <div class="mt-4">
                    ...
                    <el-input type="textarea" :value="formatJsonData(currentDetailRow.requestParams)" readonly
                        :rows="6" />
                </div>

                <div class="mt-4">
                    <h4 class="font-semibold mb-2">响应数据</h4>
                    <el-input type="textarea" :value="formatJsonData(currentDetailRow.responseData)" readonly
                        :rows="6" />
                </div>
            </div>
        </el-dialog>
        ...
        <!-- 清理历史对话框 -->
        <el-dialog v-model="cleanDialogVisible" title="清理历史日志" width="400px" :close-on-click-modal="false">
            <div>
                <p class="mb-4">请选择要保留的天数，将删除指定天数之前的所有日志记录。</p>
                <el-form>
                    <el-form-item label="保留天数">
                        <el-input-number v-model="cleanDays" :min="1" :max="365" placeholder="请输入保留天数" />
                    </el-form-item>
                </el-form>
            </div>
            <template #footer>
                <div class="flex gap-2 justify-end">
                    ...
                    <el-button type="warning" @click="confirmCleanHistory">确认清理</el-button>
                </div>
            </template>
        </el-dialog>
    </div>
...
import { tableColumnConfig, tableConfig, tableData, paginationConfig } from "./tableConfig";
import {
    searchApiLogs,
    getApiLogStatistics,
    deleteApiLog,
...
const confirmCleanHistory = async () => {
    try {
        await ElMessageBox.confirm(`确定要清理 ${cleanDays.value} 天前的历史日志吗？此操作不可恢复。`, '确认清理', {
            type: 'warning'
        });
