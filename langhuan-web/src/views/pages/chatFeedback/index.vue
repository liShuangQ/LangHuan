<template>
    <div class="box-border p-3 pb-0 pt-7 bg-white">
        ...
        <div style="height: calc(100% - 70px)" class="mt-2">
            ...
            >
                <template #content-interaction="props">
                    <el-tag
                        v-if="props.row.interaction === 'like'"
                        type="success"
                        >{{ props.row.interaction }}</el-tag
                    >
                    <el-tag
                        v-if="props.row.interaction === 'dislike'"
                        type="warning"
                        >{{ props.row.interaction }}</el-tag
                    >
                    <el-tag
                        v-if="props.row.interaction === 'end'"
                        type="info"
                        >{{ props.row.interaction }}</el-tag
                    >
                </template>
                <template #content-knowledgeBaseIds="props">
                    <el-button
                        link
                        type="primary"
                        @click="openDocumentIds(props.row)"
                        >知识库片段
                    </el-button>
                </template>
                <template #content-buttonSlot="props">
                    <el-button
                        type="primary"
                        link
                        @click="tableBtnHandle('addFile', props.row)"
                        >添加文件</el-button
                    >
                    <el-button
                        type="primary"
                        link
                        @click="tableBtnHandle('end', props.row)"
                        :disabled="
                            props.row.interaction === 'end' ||
                            props.row.interaction === 'like'
                        "
                        >标记完成</el-button
                    >
                    <el-button
                        link
                        type="primary"
                        @click="tableBtnHandle('del', props.row)"
                        >删除
                    </el-button>
                </template>
            </ElementTableC>
        ...
        <el-dialog
            v-model="documentIdsVisible"
            :title="'文档列表(修改后文档将自动归档)'"
            width="900"
        >
            <div class="h-[70vh] overflow-y-scroll">
                <div
                    v-for="(item, index) in documentIdsData"
                    :key="index"
                    class="mb-4"
                >
...
const getUserPageList = () => {
    http.request<any>({
        ...
        .then((res) => {
            ...
            if (
                search_dayTransformation &&
                search_dayTransformation.length >= 0
            ) {
...
const documentHandle = (type: string, index: number, item: any) => {
    if (type === "edit") {
        ...
        documentIdsData.value[index].tempContent =
            documentIdsData.value[index].content;
    }
    if (type === "cancel") {
...
const tableBtnHandle = (type: string, row: any) => {
    if (type === "addFile") {
        router.push({
            ...
            query: {
                tipText: `问题内容：${row.questionContent}<br/>附加建议：${row.suggestion}`,
            },
        });
    }
    if (type === "end") {
