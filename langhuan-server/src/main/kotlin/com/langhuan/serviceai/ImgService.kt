...
class ImgService {
    ...
    @Autowired
    private lateinit var imageUnderstandingProcessorFactory: ImageUnderstandingProcessorFactory
     ...
     */
    @Throws(Exception::class)
    fun chat_imageUnderstanding(chatRestOption: ChatRestOption, imageFiles: Array<MultipartFile>): ChatImageUnderstandingRes {
        val out = StringBuilder()
        val prompt = chatRestOption.userMessage + "\n" + "请使用中文回答，回答必须使用 Markdown 格式（如标题、列表、加粗等），不得嵌套任何 JSON、XML 等结构化格式，不得使用```markdown  ```代码块标记；"
        val minioChatImgs = StringBuilder()
        for (item in imageFiles) {
            // 构建地址
            val objectName = "$chatMemoryImgFold/${chatRestOption.chatId}/${UUID.randomUUID().toString()}${item.originalFilename}"
            // 上传地址
            minioService.handleUpload(objectName, item.inputStream, -1, bucket)
            // 获取minio地址
            minioChatImgs.append("![img](url)".replace("url", minioService.generateMinioUrl(objectName, bucket))).append("\n")
        }
        // 模型回答
        out.append(imageUnderstandingProcessorFactory.getProcessor().understandImage(imageFiles, prompt)).append("\n")
        val chatOutS = out.toString()

        return ChatImageUnderstandingRes().apply {
            chatInStr = minioChatImgs.toString()
            chatOutStr = chatOutS
        }
    }
     ...
     */
    @Throws(Exception::class)
    fun chat_imageUnderstandingToText(imageFiles: Array<MultipartFile>, imgPrompt: String): String {
        val prompt = """
        ...
        return imageUnderstandingProcessorFactory.getProcessor().understandImage(imageFiles, prompt)
    }

    open class ChatImageUnderstandingRes {
        var chatInStr: String = ""
        var chatOutStr: String = ""
    }
}
