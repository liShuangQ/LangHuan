...
### 核心组件
- **控制器**：位于 `controller/` 包中的 RESTful API 端点
- **服务**：位于 `service/` 和 `serviceai/` 包中的业务逻辑
- **领域模型**：位于 `model/domain/` 包中的实体类
- **安全**：基于 JWT 的 Spring Security 认证
- **RAG 系统**：位于 `serviceai/` 和 `utils/rag/` 包中的向量存储和文档检索
### 关键包
- `com.langhuan.controller/` - REST API 端点
- `com.langhuan.service/` - 核心业务服务
- `com.langhuan.serviceai/` - 与 AI 相关的服务（聊天、RAG、重排序）
- `com.langhuan.model/` - 数据模型（领域模型、数据传输对象、视图对象）
- `com.langhuan.config/` - Spring 配置类
- `com.langhuan.utils/` - 工具类和 RAG 处理工具
## 开发工作流程
### 添加新功能
1. 如果需要新的数据库表，请在 `model/domain/` 中创建实体。
2. 在 `model/mapper/` 中添加映射器接口
3. 在 `service/` 或 `serviceai/` 中创建服务
4. 在 `controller/` 中添加控制器
5. 更新 `resources/sql/` 中的数据库架构