...
public class Constant {
        ...
        public static final String[] URL_WHITELIST = { "/favicon.ico", "/user/login", "/user/register",
                        "/service/user/login", "/service/user/register", "/test/*" };
        // ai的rag默认拆分规则
        ...
        // 从向量库返回的设置返回的得分最高结果的数量
        public static final int RAGWITHTOPK = 20;
        // 从上方的结果中取前多少个结果用于下一阶段排序（例如20条结果，取前10条按rerank排序）
        // 如果开启rerank，那么这个值就是需要给rerank模型的数量
        public static final int RAGRANKTOPN = 15;
        // 是否开启rerank，当接口中没传递的时候默认使用这个值
        public static final Boolean ISRAGRERANK = false;
        // rerank的topn
        public static final int RAGRERANKTOPN = 10;
        // 经过手工排序筛选后给LLM的RAG topn。 实际输出数量。 给模型RAG的数量，也是召回等的结果，这个数量不能小于RAGRANKTOPK或RAGRERANKTOPN
