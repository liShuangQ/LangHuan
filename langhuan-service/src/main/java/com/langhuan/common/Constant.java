...
public class Constant {
    ...
    // 经过手工排序筛选后给LLM的RAG topn。 实际输出数量。 给模型RAG的数量，也是召回等的结果，这个数量不能小于RAGRANKTOPK或RAGRERANKTOPN等值
    ...
    // {从上方的结果中取前多少个结果用于下一阶段排序(例如20条结果，取前10条按rerank排序)如果开启rerank那么这个值就是需要给rerank模型的数量, rerank的topn}
